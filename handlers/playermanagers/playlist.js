...
async function playlist(client, message, args, type, slashCommand = false) {
  ...
  try {
    ...
    var playlistembed = new MessageEmbed()
      ...
      .setDescription(`A playlist \`${res.playlist.name}\` foi adicionada com ${res.tracks.length}!`)
      .setFooter({text:`Adicionada por: ${player.queue.requester.tag}`, iconURL: player.queue.requester.displayAvatarURL()})
