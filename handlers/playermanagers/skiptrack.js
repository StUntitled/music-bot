...
async function skiptrack(client, message, args, type, slashCommand) {
  ...
  async function playlist_() {
    ...
    .setDescription(`A playlist \`${res.playlist.title}\` foi adicionada com ${res.playlist.length}!`)
    .setFooter({text:`Adicionada por: ${player.queue.current.requester.tag}`, iconURL: player.queue.current.requester.displayAvatarURL()})
