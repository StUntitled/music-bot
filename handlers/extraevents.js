...
module.exports = client => {
  ...
  client.logger = (data) => {
    ...
    let logstring = `${String(`Logs Bot de Música 💿`).yellow}${` | `.grey}${`${moment().format("HH:mm:ss")}`.cyan}${` | `.grey}`
  ...
  client.editLastPruningMessage = async (player, footertext = "Música finalizada") => {
    client.logger("A editar a última mensagem System called and executed")
    let guild = client.guilds.cache.get(player.guild);
    if (!guild) return client.logger("A editar a última mensagem - Server não encontrado!")
    //try to get the channel
    ...
    if (!channel) return client.logger("A editar a última mensagem - Canal não encontrado")
    ...
    if (!message) return client.logger("A editar a última mensagem - Mensagem não encontrada!")
    if (!message.embeds || !message.embeds[0]) return client.logger("A editar a última mensagem - Embeds removidas!")
    //get the embed + change it
    var embed = message.embeds[0];
    embed.author.iconURL = "https://cdn.discordapp.com/attachments/962830738657529967/964897345093369856/1ba7f3c43c7b416afc2e7a8c39a9016d.png"
    embed.footer.text = footertext;
    embed.author.name = "Música finalizada!"
    //Edit the message
    message.edit({
      embeds: [embed],
      components: []
    ...
    if (settings.deleteMessagesBeforeTheLastSongPlayedMessages) {
      ...
      if (!beforemessage) return client.logger("A editar a última mensagem - Before - Message not found!")
      //if not able to 
      if (!beforemessage) return client.logger("A editar a última mensagem - Before - Message already deleted");
      if (!beforemessage.deletable) return client.logger("A editar a última mensagem - Before - Message not delete able");
      //delete the message
  ...
  client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
    ...
    if(!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
    if(!url || url.length < 1) url = `https://youtube.com/`;
