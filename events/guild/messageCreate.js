...
module.exports = async (client, message) => {
      try {
        ...
        if (!message.guild.me.permissions.has(Discord.Permissions.FLAGS.USE_EXTERNAL_EMOJIS))
          return message.reply(`❌ **Não tenho a permissão de usar emojis externos!**`)
        if (!message.guild.me.permissions.has(Discord.Permissions.FLAGS.EMBED_LINKS))
          return message.reply(`❌ **Não tenho a permissão para enviar embeds!**`)
        if (!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADD_REACTIONS))
          return message.reply(`❌ **Não tenho a permissão de adicionar reações!**`)
          ...
          if (matchedPrefix.includes(client.user.id))
            return message.reply({
              embeds: [new Discord.MessageEmbed()
                ...
                .setAuthor({name:`O meu prefixo aqui é \`${prefix}\` | Para ver todos os comandos escreve ${prefix}help`, iconURL: message.author.displayAvatarURL()})
              ]
            }).catch(() => {})
          return;
        }
        ...
        if (command) {
          ...
          if (musicData.channel && musicData.channel == message.channel.id) {
            return message.reply("❌ **Por favor usa o comando em qualquer outro canal!**").then(msg => {
              setTimeout(() => {
                try {
                  msg.delete().catch(() => {});
                } catch (e) {}
            ...
            if (command.memberpermissions) {
              if (!message.member.permissions.has(command.memberpermissions)) {
                ...
                message.reply({
                  embeds: [new Discord.MessageEmbed()
                    .setColor(es.wrongcolor)
                    .setAuthor({name:`Não tens permissão para usar este comando!`, iconURL: message.author.displayAvatarURL()})
        ...
        if(command.parameters) {
            if(command.parameters.type == "music"){
              ...
              if(message.member.voice.selfDeaf || message.member.voice.deaf)
                return message.reply({embeds : [new MessageEmbed()
                  .setColor(es.wrongcolor)
                  .setAuthor({name:`Não podes executar este comando enquanto estás Deafen!`, iconURL: message.author.displayAvatarURL()})
              ...
              if(player && player.queue && player.queue.current && command.parameters.check_dj){
                if(check_if_dj(client, message.member, player.queue.current)) {
                  return message.reply({embeds: [new MessageEmbed()
                    ...
                    .setAuthor({name: `Não és um DJ nem o usuário que pediu a música!`, iconURL:message.author.displayAvatarURL()})
              ...
              if(command.parameters.activeplayer){
                if (!player){
                  ...
                  return message.reply({embeds: [new MessageEmbed()
                    .setColor(es.wrongcolor)
                    .setAuthor({name:`Não estou a tocar nada no momento.`, iconURL:message.author.displayAvatarURL()})
                  ]}).catch(()=>{})
                }
                if (!mechannel){
                  ...
                  return message.reply({embeds: [new MessageEmbed()
                    .setColor(es.wrongcolor)
                    .setAuthor({name:`Não estou mais conectado a um canal de voz.`, iconURL:message.author.displayAvatarURL()})]}).catch(()=>{})
                }
                if(!player.queue || !player.queue.current){
                  return message.reply({embeds : [new MessageEmbed()
                    .setColor(es.wrongcolor)
                    .setAuthor({name:`Não há nada na fila a tocar!`, iconURL:message.author.displayAvatarURL() || i.user.displayAvatarURL()})
                  ]}).catch(()=>{})
                }
              }
              //if no previoussong
              ...
              if (player && channel.id !== player.voiceChannel && !command.parameters.notsamechannel){
                return message.reply({embeds: [new MessageEmbed()
                  .setColor(es.wrongcolor)
                  .setAuthor({name:`Precisas de estar no mesmo canal de voz que eu para usar esse comando!`, iconURL: message.author.displayAvatarURL()})
                  .setDescription(`Canal de voz: <#${player.voiceChannel}> `)
                ]}).catch(()=>{})
            }
            //if not in the same channel --> return
            if (mechannel && channel.id !== mechannel.id && !command.parameters.notsamechannel) {
              return message.reply({embeds: [new MessageEmbed()
                .setColor(es.wrongcolor)
                .setAuthor({name:`Precisas de estar no mesmo canal de voz que eu para usar esse comando!`, iconURL: message.author.displayAvatarURL()})
                .setDescription(`Canal de voz: <#${mechannel.id}>`)
              ]}).catch(()=>{})
            }
          }
        }
        ...
        return message.reply({embeds: [
          new Discord.MessageEmbed()
          .setColor(es.wrongcolor)
          .setAuthor({name:`Ocorreu um erro ao usar o comando.`, iconURL: message.author.displayAvatarURL()})
